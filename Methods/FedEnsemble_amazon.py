...
class FEA(nn.Module):
    ...
    def forward(self, users=None, pos_items=None, neg_items=None):
        ...
        for i in range(1, self.client_count):
            pos_score += (user_weight_list[i][users] * emb_item[pos_items]).sum(dim=-1)
            neg_score += (user_weight_list[i][users] * emb_item[neg_items]).sum(dim=-1)
            # pos_score_list.append(pos_score)
            # neg_score_list.append(neg_score)

        return pos_score, neg_score, user_weight_list
    ...
    def predict(self, users, items, seq):
        ...
        for i in range(self.client_count):
            ...
            user_weight_list.append(user_weight)
        for i in range(self.client_count):
            scores += (user_weight_list[i][users] * emb_item[items]).sum(dim=-1)
    ...
    def get_loss(self, users, pos_items, neg_items, seq):
        pos_score, neg_score, user_weight_list = self.forward(users, pos_items, neg_items)
        rank = torch.ones_like(pos_score)

        mse = nn.MSELoss()
        mse_loss = mse(pos_score, rank)
        bpr_loss = - (pos_score - neg_score).sigmoid().log().mean()
        ali_loss = 0
        for i in range(self.client_count):
            ali_loss += F.mse_loss(user_weight_list[i], self.emb_user.weight)
        loss = 0.01 * mse_loss + 0.8 * bpr_loss + 0.01 * ali_loss
        return loss, 0
